...
function appSnapshot(id){const t=targets[id];return{id,...t,deployed:readJson(path.join(t.root,'data','deployed-version.json')),deploy:readJson(path.join(t.root,'data','deploy-status.json'))||{state:'idle'},autoDeploy:readJson(path.join(t.root,'data','auto-deploy-status.json'))};}
